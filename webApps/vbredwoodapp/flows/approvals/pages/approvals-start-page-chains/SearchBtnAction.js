define([
  'vb/action/actionChain',
  'vb/action/actions',
  'vb/action/actionUtils',
], (
  ActionChain,
  Actions,
  ActionUtils
) => {
  'use strict';

  class SearchBtnAction extends ActionChain {

    /**
     * @param {Object} context
     */
    async run(context) {
      const { $page, $flow, $application, $constants, $variables, $functions } = context;

      const loadingDialogOpen = await Actions.callComponentMethod(context, {
        selector: '#loadingDialog',
        method: 'open',
      });

      const response = await Actions.callRest(context, {
        endpoint: 'PWP_ORDS/getLienManagement_Approval_Search',
        uriParams: {
          'p_ap_invoice': $variables.headers.apInvoice? $variables.headers.apInvoice:'',
          'p_project_number': $variables.headers.project? $variables.headers.project:'',
          'p_supplier_name': $variables.headers.supplier?$variables.headers.supplier:'',
          'p_attchment_status': $variables.headers.status ? $variables.headers.status :'',
          USERNAME: $application.variables.user,
        },
      });

      if (!response.ok) {
         const loadingDialogClose = await Actions.callComponentMethod(context, {
        selector: '#loadingDialog',
        method: 'close',
      });
      }

      const addPaymentTypeTemporarily = await $functions.addPaymentTypeTemporarily(response.body.items);

      const loadingDialogClose = await Actions.callComponentMethod(context, {
        selector: '#loadingDialog',
        method: 'close',
      });

      $variables.approvalADP.data = addPaymentTypeTemporarily;

      await Actions.fireDataProviderEvent(context, {
        target: $variables.approvalADP,
        refresh: null,
      });
    }
  }

  return SearchBtnAction;
});
